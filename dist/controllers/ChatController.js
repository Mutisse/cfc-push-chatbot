...
const MenuManager_1 = require("../models/MenuManager");
class ChatController {
    async handleWebhook(req, res) {
        try {
            ...
            logger_1.logger.info(`📱 Mensagem de ${userPhone}: "${userMessage}"`);
            // ✅ VALIDAÇÃO: Ignorar mensagens do próprio sistema Twilio
            if (userPhone === "+14155238886" ||
                !userMessage ||
                userMessage === "undefined" ||
                userMessage.trim() === "") {
                logger_1.logger.warn(`⚠️ Mensagem inválida ignorada de: ${userPhone}`);
...
<Response>
            ...
            const userSession = await UserStateService_1.UserStateService.getState(userPhone);
...
<Response>
            ...
            res.type("text/xml");
            res.send(emptyResponse);
            logger_1.logger.info(`✅ Resposta enviada via WhatsApp para ${userPhone}: ${currentStep}`);
        }
        catch (error) {
            ...
            try {
                const userPhone = req.body.From
                    ...
                    : "unknown";
                await WhatsAppService_1.WhatsAppService.sendTextMessage(userPhone, "❌ Desculpe, ocorreu um erro no servidor. Por favor, tente novamente.");
            }
            catch (whatsappError) {
                logger_1.logger.error("❌ Erro ao enviar mensagem de erro via WhatsApp:", whatsappError);
            }
...
<Response>
    ...
    async sendWhatsAppMessage(phone, text, buttons) {
        try {
            if (buttons && buttons.length > 0) {
                // Se tem botões, enviar mensagem interativa
                await WhatsAppService_1.WhatsAppService.sendInteractiveButtons(phone, text, buttons);
            }
            else {
                // Se não tem botões, enviar mensagem de texto simples
                await WhatsAppService_1.WhatsAppService.sendTextMessage(phone, text);
            }
        }
        catch (error) {
            logger_1.logger.error(`❌ Erro ao enviar mensagem WhatsApp para ${phone}:`, error);
            throw error;
        }
    }
    ...
    async sendWelcomeMessage(phone) {
        try {
            await WhatsAppService_1.WhatsAppService.sendWelcomeMessage(phone);
            // Aguardar 2 segundos e enviar o menu principal
            setTimeout(async () => {
                const welcomeResponse = await this.handleWelcome("", phone);
                await this.sendWhatsAppMessage(phone, welcomeResponse.text, welcomeResponse.buttons);
            }, 2000);
        }
        catch (error) {
            logger_1.logger.error(`❌ Erro ao enviar mensagem de boas-vindas para ${phone}:`, error);
        }
    }
    // 🎯 MENSAGEM DE BOAS-VINDAS COM BOTÕES
    async handleWelcome(message, phone) {
        await UserStateService_1.UserStateService.setState(phone, "MAIN_MENU");
    ...
    async handleMainMenu(message, phone) {
        ...
        if (this.isValidMenuOption(normalizedMessage, 1, 15)) {
            switch (normalizedMessage) {
                ...
                case "6":
                    await UserStateService_1.UserStateService.setState(phone, "VISITA_DATA");
                    return {
                ...
                case "14":
                    await UserStateService_1.UserStateService.setState(phone, "PUSH_INVEST_MENU");
                    return await this.handlePushInvestMenu("", phone);
                case "15":
                    await UserStateService_1.UserStateService.deleteState(phone);
                    return {
    ...
    async handleSerMembroSubmenu(phone) {
        const buttons = [
    ...
    async handlePushInvestMenu(message, phone) {
        const normalizedMessage = message.trim();
        ...
        if (normalizedMessage &&
            ...
            await UserStateService_1.UserStateService.setState(phone, opcoesPushInvest[opcao]);
            switch (opcao) {
                case "projetos":
                    return {
    ...
    async handlePushInvestProjetos(message, phone) {
        await UserStateService_1.UserStateService.resetToMainMenu(phone);
    ...
    async handlePushInvestInvestir(message, phone) {
        await UserStateService_1.UserStateService.resetToMainMenu(phone);
    ...
    // 🎯 PUSH INVEST - CONTATO
    async handlePushInvestContato(message, phone) {
        await UserStateService_1.UserStateService.resetToMainMenu(phone);
    ...
    async handleNucleoRegiao(message, phone) {
        const regions = {
            "Zona Norte": {
    ...
    async handleMinisterioTipo(message, phone) {
        const ministerios = {
            "Louvor e Adoração": {
    ...
    async handleOracaoTipo(message, phone) {
        const tiposOracao = {
            Saúde: "saude",
            Família: "familia",
            Finanças: "financas",
        ...
        if (message && tiposOracao[message]) {
            await UserStateService_1.UserStateService.updateData(phone, {
                prayerType: tiposOracao[message],
            });
            if (message === "Família") {
                await UserStateService_1.UserStateService.setState(phone, "ORACAO_NOME_FAMILIA");
                return {
                ...
                await UserStateService_1.UserStateService.setState(phone, "ORACAO_DETALHE");
                return {
    ...
    async handleAssistenciaTipo(message, phone) {
        ...
        if (message && tiposAssistencia[message]) {
            ...
            await UserStateService_1.UserStateService.setState(phone, "ASSISTENCIA_DETALHE");
            return {
    ...
    // 🎯 CADASTRO - NOME
    async handleCadastroNome(message, phone) {
        ...
        await UserStateService_1.UserStateService.updateData(phone, { name: message.trim() });
        await UserStateService_1.UserStateService.setState(phone, "CADASTRO_DATA_NASCIMENTO");
        return `✅ Nome válido: *${message.trim()}*\n\nAgora digite sua *data de nascimento*:\n(ex: 15/08/1990)`;
    }
    // 🎯 CADASTRO - DATA NASCIMENTO
    async handleCadastroDataNascimento(message, phone) {
        if (!this.isValidMessage(message)) {
        ...
        await UserStateService_1.UserStateService.updateData(phone, { dateOfBirth: message });
        await UserStateService_1.UserStateService.setState(phone, "CADASTRO_ESTADO_CIVIL");
        return `✅ Data válida!\n\nQual seu *estado civil*?\n\n[1] Solteiro(a)\n[2] Casado(a)\n[3] Divorciado(a)\n[4] Viúvo(a)\n[5] União de Facto`;
    }
    // 🎯 CADASTRO - ESTADO CIVIL
    async handleCadastroEstadoCivil(message, phone) {
        const estados = {
            "1": "Solteiro(a)",
            "2": "Casado(a)",
            "3": "Divorciado(a)",
        ...
        if (estados[message]) {
            ...
            await UserStateService_1.UserStateService.setState(phone, "CADASTRO_ENDERECO");
    ...
    // 🎯 CADASTRO - ENDEREÇO
    async handleCadastroEndereco(message, phone) {
        ...
        await UserStateService_1.UserStateService.updateData(phone, { address: message.trim() });
        await UserStateService_1.UserStateService.setState(phone, "CADASTRO_PROFISSAO");
        return `✅ Endereço registado!\n\nQual sua *profissão* ou *ocupação*?`;
    }
    // 🎯 CADASTRO - PROFISSÃO
    async handleCadastroProfissao(message, phone) {
        if (!this.isValidMessage(message)) {
            return "❌ Profissão vazia. Digite sua profissão:";
        }
        await UserStateService_1.UserStateService.updateData(phone, { profession: message.trim() });
        await UserStateService_1.UserStateService.setState(phone, "CADASTRO_COMO_CONHECEU");
        return `✅ Profissão registada: *${message.trim()}*\n\n*Como conheceu a CFC PUSH?*\n\n[1] Amigo/Familiar\n[2] Rede Social\n[3] Visita/Evento\n[4] Propaganda\n[5] Outro`;
    }
    // 🎯 CADASTRO - COMO CONHECEU
    async handleCadastroComoConheceu(message, phone) {
        const opcoes = {
            "1": "Amigo/Familiar",
            "2": "Rede Social",
            "3": "Visita/Evento",
        ...
        if (opcoes[message]) {
            const session = await UserStateService_1.UserStateService.getState(phone);
            const userData = session?.data || {};
            // ✅ SALVAR CADASTRO COMPLETO
            try {
                await UserStateService_1.UserStateService.saveUser(phone, {
                    name: userData.name,
                ...
                logger_1.logger.info(`✅ Novo membro cadastrado: ${userData.name} (${phone})`);
            }
            catch (error) {
                logger_1.logger.error("❌ Erro ao salvar cadastro:", error);
            }
            await UserStateService_1.UserStateService.resetToMainMenu(phone);
    ...
    async handleOracaoNomeFamilia(message, phone) {
        if (!this.isValidMessage(message)) {
    ...
    // 🎯 ORAÇÃO - DETALHE
    async handleOracaoDetalhe(message, phone) {
        ...
        await UserStateService_1.UserStateService.updateData(phone, { prayerDetail: message.trim() });
        await UserStateService_1.UserStateService.setState(phone, "ORACAO_ANONIMATO");
    ...
    // 🎯 ORAÇÃO - ANONIMATO
    async handleOracaoAnonimato(message, phone) {
        const session = await UserStateService_1.UserStateService.getState(phone);
        const userData = session?.data || {};
        ...
        if (message === "2") {
            try {
                const user = await UserStateService_1.UserStateService.getUser(phone);
    ...
    async handleAssistenciaDetalhe(message, phone) {
        ...
        try {
            ...
            logger_1.logger.info(`🤝 Assistência solicitada: ${userName} - ${userData.assistanceType} - ${message}`);
        }
        catch (error) {
            logger_1.logger.error("❌ Erro ao salvar assistência:", error);
        }
        await UserStateService_1.UserStateService.resetToMainMenu(phone);
    ...
    // 🎯 VISITA - DATA
    async handleVisitaData(message, phone) {
        const validation = this.isValidFutureDate(message);
        if (!validation.isValid) {
            return `${validation.error}\n\n*Digite a data novamente:* (ex: 25/12/2024)`;
        }
        await UserStateService_1.UserStateService.updateData(phone, { visitDate: message });
        await UserStateService_1.UserStateService.setState(phone, "VISITA_MOTIVO");
        return `✅ Data válida: *${message}*\n\nQual o *motivo da visita pastoral*?`;
    }
    // 🎯 VISITA - MOTIVO
    async handleVisitaMotivo(message, phone) {
        if (!this.isValidMessage(message) || message.trim().length < 5) {
        ...
        try {
            ...
            logger_1.logger.info(`🏠 Visita solicitada: ${userName} - ${visitDate} - ${message}`);
        }
        catch (error) {
            logger_1.logger.error("❌ Erro ao salvar visita:", error);
        }
        await UserStateService_1.UserStateService.resetToMainMenu(phone);
    ...
    async handleTransferenciaIgrejaOrigem(message, phone) {
        if (!this.isValidMessage(message)) {
        ...
        await UserStateService_1.UserStateService.setState(phone, "TRANSFERENCIA_MOTIVO");
        return `✅ Igreja de origem registada: *${message.trim()}*\n\nQual o *motivo da transferência*?`;
    }
    // 🎯 TRANSFERÊNCIA - MOTIVO
    async handleTransferenciaMotivo(message, phone) {
        if (!this.isValidMessage(message)) {
        ...
        try {
            ...
            logger_1.logger.info(`🔄 Transferência solicitada: ${userName} - ${userData.previousChurch} - ${message}`);
        }
        catch (error) {
            logger_1.logger.error("❌ Erro ao processar transferência:", error);
        }
        await UserStateService_1.UserStateService.resetToMainMenu(phone);
    ...
    async handleAtualizacaoDadosTipo(message, phone) {
        const opcoes = {
            "1": "name",
            "2": "address",
            "3": "phone",
        ...
        if (opcoes[message]) {
            ...
            await UserStateService_1.UserStateService.setState(phone, "ATUALIZACAO_DADOS_NOVO_VALOR");
            const fieldNames = {
                name: "Nome",
                address: "Endereço",
                phone: "Telefone",
            ...
            return `✅ Campo selecionado: *${fieldNames[opcoes[message]]}*\n\nDigite o *novo valor* para ${fieldNames[opcoes[message]].toLowerCase()}:`;
        }
        else {
    ...
    async handleAtualizacaoDadosNovoValor(message, phone) {
        if (!this.isValidMessage(message)) {
        ...
        try {
            ...
            if (user) {
                ...
                await UserStateService_1.UserStateService.saveUser(phone, updateData);
                logger_1.logger.info(`✏️ Dados atualizados: ${phone} - ${userData.updateField}: ${message}`);
            }
        }
        catch (error) {
            logger_1.logger.error("❌ Erro ao atualizar dados:", error);
        }
        await UserStateService_1.UserStateService.resetToMainMenu(phone);
        const fieldNames = {
            name: "Nome",
        ...
        return `✅ *DADOS ATUALIZADOS COM SUCESSO!*\n\n*Campo:* ${fieldNames[userData.updateField || "name"]}\n*Novo valor:* ${message}\n\nSeus dados foram atualizados em nosso sistema.\n\nDigite [#] para menu principal.`;
    }
    // 🎯 DIREITOS E DEVERES
    async handleDireitosDeveres(message, phone) {
        await UserStateService_1.UserStateService.resetToMainMenu(phone);
    ...
    isValidMenuOption(option, min = 1, max = 15) {
    ...
    formatMainMenuText() {
        const menu = MenuManager_1.MenuManager.getMainMenu();
